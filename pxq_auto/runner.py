...
from .config import AppConfig, account_home
...
async def run_account(config: AppConfig, context, *, presale: bool) -> RunResult:
    ...
    if not config.safety.create_order:
    ...
    auth = AuthGuard(site)
    try:
        await auth.ensure(interactive=False)
        ...
        if presale:
            ...
            if not sale.on_sale:
                ...
                with suppress(StaticInventoryUnavailable):
                    source = await bootstrap.activate()
                if not sale.on_sale:
            ...
            async def wait_selection():
                current = source or await bootstrap.wait_static()
            ...
            source = await Inventory.open(site, auth)
            selection = await source.refresh(len(people))
        selected_people = people[: len(selection.candidates)]
        prepared = await site.prepare_order(selection, selected_people)
    except AuthenticationError:
        return RunResult("NEEDS_LOGIN", "登录状态已失效")
    except InventoryUnavailable:
        return RunResult("NO_STOCK", "实时座位库存已售空")
    except Exception:
    ...
    while True:
        ...
        try:
            _, selection = await asyncio.gather(
                site.reopen_seat_map(), source.refresh(len(people))
            )
            selected_people = people[: len(selection.candidates)]
            prepared = await site.prepare_order(
                selection, selected_people, open_map=False
            )
        except AuthenticationError:
            return RunResult(
                "NEEDS_LOGIN",
...
async def _save_failure(site: PiaoxingqiuPage, config: AppConfig, name: str) -> None:
    with suppress(Exception):
        directory = account_home(config.browser.profile_dir.parent.name) / "artifacts"
